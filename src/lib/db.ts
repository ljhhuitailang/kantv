...
// 创建存储实例
function createStorage(d1Binding?: any): IStorage {
  switch (STORAGE_TYPE) {
    case 'redis':
    ...
    case 'd1':
      return new D1Storage(d1Binding);
    case 'localstorage':
    default:
...
function getStorage(d1Binding?: any): IStorage {
  if (!storageInstance) {
    storageInstance = createStorage(d1Binding);
  }
...
export class DbManager {
  ...
  constructor(d1Binding?: any) {
    this.storage = getStorage(d1Binding);
  }
